...
raw_config_loader.py	This reads the contents of the excel config file. The excel config file described the mapping
                        between the tags in OSM data and the attributes that the output shapefiles will have. No
                        manipulation of this table occurs here.

configengine.py			This takes the table from from the excel config file and loads it into an in-memory SQLite DB.
                        There it is normalised and manipulated to produce an output table with exactly one row per
                        attribute in each output shapefile.

ogrwrapper.py           Takes the shapefile list and drives the OGR libraries in order to actually generate the output
